...
menuItemsRouter.param("menuItemId", (req, res, next, menuItemId) => {
  db.get(`SELECT * FROM MenuItem WHERE id=${menuItemId}`, (error, data) => {
    if (error) {
      next(error);
    } else if (data) {
      req.menuItem = data;
...
menuItemsRouter.post("/", validateFields, (req, res, next) => {
  db.run(
    `INSERT INTO MenuItem (name, description, inventory, price, menu_id) VALUES ("${req.name}", "${req.description}", ${req.inventory}, ${req.price}, ${req.params.menuId})`,
    (error, data) => {
      if (error) {
        next(error);
      } else {
...
menuItemsRouter.put("/:menuItemId", validateMenuItem, (req, res, next) => {
  db.run(
    `UPDATE MenuItem SET name = "${req.name}", description = "${req.description}",
...
menuItemsRouter.put("/:menuItemId");
